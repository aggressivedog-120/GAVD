...
p = paras['p']
r = paras['r']
seeds = [22, 919, 815, 218, 369, 735, 248, 264, 350, 34]
# 1. 生成训练数据集
...
for i in range(len(rs)):
    ...
    data_test_dict[test_r] = temp


# 3.开始循环，for每个训练数据集
results_in_diff_data = [] # 记录在不同训练数据集下的结果
for data_train in data_train_list[:1]:
    ...
    alpha=0.5
    n_iter = 5
    w_moment = 0.3  # 新的w和之前的w进行加权产生下一届的w
    w_list = []
    loss_lists = []
    acc_list = []
    ...
    for i_iter in range(n_iter):  # run n_iter times to update weight w
        ...
        W1 = tf.Variable(tf.random_normal([n_x, 10], stddev=0.001, seed=1))
        b1 = tf.get_variable("b1", [1, 10], initializer=tf.zeros_initializer())
        W2 = tf.Variable(tf.random_normal([10, 1], stddev=0.001, seed=1))
        ...
        element_loss = -((0.5+0.5*Y) * tf.log(tf.clip_by_value(A3, 1e-10, 1.0)) + (0.5 - 0.5*Y) * tf.log(1 - tf.clip_by_value(A3, 1e-10, 1.0)))
        entropy = -tf.reduce_mean(tf.matmul(sample_weight,
                  (0.5 + 0.5 * Y) * tf.log(tf.clip_by_value(A3, 1e-10, 1.0)) + (0.5 - 0.5*Y) * tf.log(1 - tf.clip_by_value(A3, 1e-10, 1.0))))  # with reweight
        loss = entropy
    ...
    plt.colorbar()
    plt.title('corr without weight n={}_p={}_r={}'.format(real_n, p, r))
    plt.show()

    plt.figure()
    plt.imshow(corr_weighted, cmap=plt.cm.Reds, origin='lower')
    plt.colorbar()
    plt.title('corr with weight n={}_p={}_r={}'.format(real_n, p, r))
    plt.show()
    ...
    plt.ylabel('RMSE')
    plt.title('r on test data n={}_p={}_r={}'.format(real_n, p, r))
    #plt.savefig('result/rmse1.png')
    plt.show()

    temp_result = (OLS_rcd, Lasso_rcd, Ridge_rcd, Gan_rcd)
    results_in_diff_data.append(temp_result)
